...
The daily sweep (`runResearchSweep`) is the marketing team's framework: one ordered daily job with 3 fire-and-forget stages — **(1) RISET feed → (2) MATERI IKLAN per platform → (3) SEQUENCE RETENSI (email/WA)**. Each stage isolated in its own try/catch so a partial failure never breaks the rest. Full detail (agents, roles, KB prune-scopes): `docs/marketing-pipeline.md`.

**Durable design rules for this pipeline:**
- **Chaining by return value:** a later stage consumes an earlier stage's generated content (ad-materials output feeds the retention stage). When adding a stage, return the generated doc so the next stage can ground on it — don't re-read the DB.
- **Grounding = static foundation + fresh output.** The retention stage is grounded on a canonical foundation doc (AI Organization Builder vision + Trilogi "Dari Monolog ke Dialog" + produk/jasa) seeded idempotently, PLUS the day's marketing output. Keep the foundation doc as the single honest source for vision/product claims in customer comms.
- **Retention never auto-sends.** It drafts email/WA sequences only; sending is ◆ gerbang manusia (founder decides) even though a Brevo send helper exists. Do not wire auto-send.
