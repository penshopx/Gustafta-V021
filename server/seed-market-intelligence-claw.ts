...
export async function seedMarketIntelligenceClaw() {
  ...
  for (const sa of MI_SUB_AGENTS) {
    ...
    const created = await storage.createAgent({ name: sa.name, slug: sa.slug, description: `Divisi Market Intelligence: ${sa.role}`, systemPrompt: sa.systemPrompt, aiModel: "gpt-4o-mini", temperature: "0.3", maxTokens: 2000, isPublic: false, isActive: true, tagline: sa.role, avatar: "🎯", agenticSubAgents: null } as any);
